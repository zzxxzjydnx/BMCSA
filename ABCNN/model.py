...
class ABCNN(nn.Module):
    ...
    def __init__(self, embeddings, num_layer=1, linear_size=300, max_length=50, device="gpu"):
        ...
        self.convs = nn.ModuleList([nn.Conv2d(in_channels=1, out_channels=200, kernel_size=(k,k)) for k in (3,4,5)])
    ...
    def forward(self, q1, q2):
        ...
        #print("x is shape",x.size())
        new_x_shape = x.size()[:-1] + (12, 100)
    ...
    def conv_and_pool(self, x, conv):
    		...
    		x = F.max_pool2d(x, kernel_size=(4,size))
    		x = x.squeeze(3)
    		x = x.view(x.size(0),-1)
    		return x
