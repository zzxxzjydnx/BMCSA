...
def validate(model, dataloader, criterion):
    ...
    with torch.no_grad():
        for (q, _, h, _, label) in dataloader:
            ...
            _, predit = probs.max(dim=1)
            all_prob.extend(probs[:,1].cpu().numpy())
            all_labels.extend(label.cpu().detach().numpy())
            results.extend(predit.cpu().detach().numpy())
    
    acc = accuracy_score(all_labels, results)
    report = metrics.classification_report(all_labels, results, digits=4)
    
    precision_scores = precision_score(all_labels,results,average="weighted")
    recall_scores = recall_score(all_labels,results,average="weighted")
    f1_scores = f1_score(all_labels,results,average="weighted")      
    epoch_time = time.time() - epoch_start
    epoch_loss = running_loss / len(dataloader)
    
    #epoch_accuracy = running_accuracy / (len(dataloader.dataset))
    
    return acc,report,precision_scores,recall_scores,f1_scores,epoch_loss
