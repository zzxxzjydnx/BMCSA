...
class BIMPM(nn.Module):
    def __init__(self, embeddings, hidden_size=100, num_perspective=20, class_size=3, device="gpu"):
        ...
        self.device = device
        self.convs = nn.ModuleList([nn.Conv2d(in_channels=1, out_channels=200, kernel_size=(k,k)) for k in (2,3,4)])
        ...
        # ----- Prediction Layer -----
        self.pred_fc1 = nn.Linear((self.hidden_size * 4)*3, self.hidden_size * 4)
        self.pred_fc2 = nn.Linear(self.hidden_size * 4, self.class_size)
    ...
    def conv_and_pool(self, x, conv):
    		...
    		channel=x.size(2)
    		if channel ==5:
    			x = F.max_pool2d(x, kernel_size=(3,size+2),padding=1)
    		else:
    			k_size = int(channel/3)
    			x = F.max_pool2d(x, kernel_size=(3,size))
    		x = x.squeeze(3)
    		x = x.view(x.size(0),-1)
    		return x
    def softatt(self, x,query):
    		d_k = query.size(-1)
    ...
    def forward(self, q1, q2):
        ...
        new_x_shape = x.size()[:-1] + (8, 150)
        x = x.view(*new_x_shape)
        query = self.dropout(x)
        context,score = self.softatt(x,query)
        x = context.view(context.size(0),-1)
